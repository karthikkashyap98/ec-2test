...
#define paths
USER_PIC_DIR = os.path.join(MEDIA_ROOT, "users")
...
def retrain():
	...
	return (("{0} New users added".format(updated_users_count-prev_users_count)), "Retrained {0} users and updated {1} encodings in {2}".format(updated_users_count, len(knownEncodings), datetime.now()-start))